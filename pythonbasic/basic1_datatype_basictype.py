...
print(str1_4.find('py')+str1_4.find('ss')+str1_4.find('h',3))
        #count() 返回指定字符串在目标字符串中出现的次数
            #格式 str.count('sub'[,start[,end]])
print(str1_4.count('h')-str1_4.count('o',7))
        #index() 同find()，但要查找的内容不存在时会抛出异常，rindex()，同index()，但从右边开始查找
...
print(str1_4)
    #操作元素返回字符串
        #split()以分隔符分割字符串，返回一个列表
        #格式str.split([sep[,maxsplit]])，
        #不指定分隔符sep时按空白符(' '、\n、\t)分割且连接在一期的空白符只分割一次；指定则遇到一次分割一次
        #maxsplit，最多分割次数
print(str1_4.split(' ',1))
        #join()返回以分隔符分隔合并的字符串
        #格式：sep.join(iterable)
print(' '.join(['555','aaa']))
print(range(1,10,3))  #range() 返回的是一个可迭代对象
        #strip()，去掉首位的指定符号，不指定则去掉空白符，指定则去掉指定字符串
        #左侧？lstrip()，右侧？rstrip()
print(str1_4.strip())
print(str1_4.strip('*'))
print(str1_4.strip('\n'))
...
print('不同编码规则：',remmf_engbk.decode(encoding='utf-8',errors='replace'))  #不同编码规则
        #ps：python中字符串str有两种形式：
            ...
            #和bytes（二进制数据，在网络传输和磁盘储存用bytes类型： b'rem\xca\xc7\xce\xd2\xc0\xcf\xc6\xc5'）
